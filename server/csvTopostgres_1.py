...
def create_psam_p36_table():
    try:
        # Load the CSV file into a pandas DataFrame with all columns as strings
        df = pd.read_csv('database/psam_p36.csv', dtype=str, low_memory=False)
        ...
        if not table_exists:
            # Create the 'psam_p36' table with all columns as TEXT
        ...
        # # Change the data type of the first column to integer
        # pg_cursor.execute("""
        #     ALTER TABLE avl_modeler_datasets.psam_p36 
        #     ALTER COLUMN index TYPE integer USING (index::integer);
        # """)
        # pg_conn.commit()
